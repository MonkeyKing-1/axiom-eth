...
fn mpt_input(path: impl AsRef<Path>, slot_is_empty: bool, max_depth: usize, max_key_byte_len: usize, key_byte_len: Option<usize>) -> MPTKeyInput {
    ...
    let path = match key_byte_len {
        Some(_key_byte_len) => PathType::Var(from_hex(&key_bytes_str)),
        None => PathType::Fixed(H256(keccak256(from_hex(&key_bytes_str))))
    };
    ...
    let value = from_hex(&value_bytes_str).to_vec();
...
// // The input file is generated by running `query_test.sh` in the `scripts/input_gen` directory of this repo
// fn mpt_input_var(path: impl AsRef<Path>, slot_is_empty: bool, max_depth: usize, max_key_byte_len: usize, key_byte_len: Option<usize>) -> MPTVarKeyInput {
//     /*let block: serde_json::Value =
//     serde_json::from_reader(File::open("scripts/input_gen/block.json").unwrap()).unwrap();*/
...
//     let key_bytes_str: String = serde_json::from_value(storage_pf["key"].clone()).unwrap();
//     ///let path = keccak256(from_hex(&key_bytes_str));
//     let path = from_hex(&key_bytes_str);
//     let value_bytes_str: String = serde_json::from_value(storage_pf["value"].clone()).unwrap();
//     let value_bytes_str = if value_bytes_str.len() % 2 == 1 {
//         format!("0{}", &value_bytes_str[2..])
//     } else {
...
fn default_input() -> MPTKeyInput {
    mpt_input("scripts/input_gen/default_storage_pf.json", false, 8, 32, Some(32))
}
...
pub fn test_mock_mpt_inclusion_fixed() {
    ...
    let k = params.degree;
    let input = mpt_input("scripts/input_gen/default_storage_pf.json", false, 5, 32, Some(32)); // depth = max_depth
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();

    let input = mpt_input("scripts/input_gen/default_storage_pf.json", false, 6, 32, Some(32)); // depth != max_depth
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
}
...
pub fn test_mpt_noninclusion_branch_fixed() {
    let params = EthConfigParams::from_path("configs/tests/mpt.json");
    let k = params.degree;
    let input = mpt_input("scripts/input_gen/pos_data/noninclusion_branch_pf.json", true, 5, 32, Some(32));
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
}
...
pub fn test_mpt_noninclusion_extension_fixed() {
    let params = EthConfigParams::from_path("configs/tests/mpt.json");
    let k = params.degree;
    let input = mpt_input("scripts/input_gen/noninclusion_extension_pf.json", true, 6, 32, Some(32)); // require depth < max_depth
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
}
...
pub fn test_mpt_empty_root() {
    let params = EthConfigParams::from_path("configs/tests/mpt.json");
    let k = params.degree;
    let input = mpt_input("scripts/input_gen/empty_storage_pf.json", true, 5, 32, Some(32));
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
}
